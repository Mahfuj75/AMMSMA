...
public class AddCbsAndSavingAccountCustomAdapterForListView extends ArrayAdapter<AccountForDailyTransaction> {
    ...
    public AddCbsAndSavingAccountCustomAdapterForListView(Context context, ArrayList<AccountForDailyTransaction> memberDetailsInfoList, DataChangeListener listener) {
    ...
    public View getView(final int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        SpannableStringBuilder titleTextSpan = new SpannableStringBuilder(titleCase(accountForDailyTransaction.getProgramName()));
        ...
        if (accountForDailyTransaction.getFlag() == 0) {
            ...
            viewHolder.buttonLtsDelete.setOnClickListener(v -> {
                ...
                String cbsOrSaving = "";
                if (accountForDailyTransaction.getProgramId() > 300 && accountForDailyTransaction.getProgramId() < 400) {
                    cbsOrSaving = "CBS";
                }
                ...
                final String finalCbsOrSaving = cbsOrSaving;
                builder.setMessage(
                        "Are you sure you want to delete this " + cbsOrSaving + " account ?")
                        .setCancelable(false)
                        .setTitle("Delete")
                        .setPositiveButton("Yes",
                                (dialog, id) -> {
                                    Toast.makeText(context.getApplicationContext(), finalCbsOrSaving + " Account Deleted Successfully", Toast.LENGTH_LONG).show();
                                    if (listener != null) {
